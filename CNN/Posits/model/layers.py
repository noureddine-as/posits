#https://github.com/gary30404/convolutional-neural-network-from-scratch-python/blob/master/model/layers.py
import numpy as np
...
class Convolution2D:
    ...
    def __init__(self, inputs_channel, num_filters, kernel_size, padding, stride, learning_rate, name):
        ...
        # Posit conversion
        self.weights = posify(self.weights)
        self.bias = posify(self.bias)
        self.lr = posify(self.lr)

    def zero_padding(self, inputs, size):
        w, h = inputs.shape[0], inputs.shape[1]
        new_w = 2 * size + w
        new_h = 2 * size + h
        out = posify(np.zeros((new_w, new_h)))
    ...
    def forward(self, inputs):
        ...
        # Posit conversion
        self.inputs = posify(self.inputs)
        
        WW = (W - self.K)//self.s + 1
        HH = (H - self.K)//self.s + 1
        
        feature_maps = posify(np.zeros((self.F, WW, HH)))
        for f in range(self.F):
            for w in range(WW):
                for h in range(HH):
#                    feature_maps[f,w,h]=np.sum(self.inputs[:,w:w+self.K,h:h+self.K]*self.weights[f,:,:,:])+self.bias[f]
#                    feature_maps[f,w,h]=Posit.fused_sum((Posit.fused_matmult(self.inputs[:,w:w+self.K,h:h+self.K], self.weights[f,:,:,:])).reshape((C*self.K*self.K)))+self.bias[f]
                    feature_maps[f,w,h]=Posit.fused_sum((self.inputs[:,w:w+self.K,h:h+self.K] * self.weights[f,:,:,:]).reshape((C*self.K*self.K)))+self.bias[f]
...
class Maxpooling2D:
    ...
    def forward(self, inputs):
        ...
        out = posify(np.zeros((C, new_width, new_height)))
        for c in range(C):
...
#        return np.dot(self.inputs, self.weights) + self.bias.T
        return Posit.fused_matmult(self.inputs, self.weights) + self.bias.T
...
#        self.out = exp/np.sum(exp)
        exp = np.e**inputs        
        self.out = exp/Posit.fused_sum((exp).reshape(inputs.shape[1]))
        return self.out
    def backward(self, dy):
