#https://github.com/gary30404/convolutional-neural-network-from-scratch-python/blob/master/model/network.py
import numpy as np
import pickle 
import sys
from time import *
    ...
    def train(self, training_data, training_label, batch_size, epoch, weights_file):
        ...
        for e in range(epoch):
            for batch_index in range(0, training_data.shape[0], batch_size):
                ...
                hrs = remain_time/3600
                mins = remain_time/60-hrs*60
                secs = remain_time-mins*60-hrs*3600
                # result
                loss /= batch_size
    ...
    def test(self, data, label, test_size):
        ...
        for i in range(test_size):
            ...
            if np.argmax(output) == np.argmax(y):
                total_acc += 1
        sys.stdout.write("\n")
        print('=== Test Size:{0:d} === Test Acc:{1:.2f} ==='.format(test_size, total_acc/test_size))
